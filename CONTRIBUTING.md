# Contributing to ep_offline_edit

Thanks for helping improve `ep_offline_edit` — a plugin for [Etherpad](https://etherpad.org/). LLM/Agent contributions are explicitly welcome, provided they follow the rules below.
...
`ep_offline_edit` is a plugin, so you develop it inside a checkout of [ether/etherpad](https://github.com/ether/etherpad).
...
# Install ep_offline_edit from your local clone
pnpm run plugins i --path ../ep_offline_edit

# Start the dev server (port 9001)
pnpm --filter ep_etherpad-lite run dev
```

Edits in `../ep_offline_edit/` are picked up after a server restart.
...
* Etherpad version, ep_offline_edit version, browser + OS
* Steps to reproduce
